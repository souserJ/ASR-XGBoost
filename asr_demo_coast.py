...
【本文件 = asr_demo_coast.py】
  由 asr_demo_modified.py 复制而来，只新增一个「海陆几何」因子 COASTS：
    coast ∈ {vertical 纵向海峡(原默认), horizontal 横向海湾,
             archipelago 近海群岛, none 无海}
  默认每个 level 是固定模板（模板参数不随重复变化），差异可干净归因到几何本身；
  把 cfg['coast_jitter'] 置 True 可用 rng 抖动模板参数（每重复一套略有差异的海岸）。
  其余（环境场/适宜性/损失/门控/分区）一律不动；asr_demo_modified.py 保持不变。
...
def nonstationary_matern_field(n, rng, short_scale=5.0, long_scale=25.0):
    """Non-stationary spatial field used to test adaptive smoothing.

    The correlation length varies gradually across space: smooth regions have
    long-range correlation while heterogeneous regions contain finer-scale
    variation. This scenario prevents the simulation from favoring a single
    global smoothing strength.
    """
    yy, xx = np.mgrid[0:n, 0:n]
    weight = xx / max(1, n-1)
    large = matern_field(n, rng, long_scale)
    small = matern_field(n, rng, short_scale)
    f = (1-weight) * large + weight * small
...
COASTS = {
    'vertical':    '纵向海峡（原默认：南北向、正弦叠加蜿蜒的海峡）',
    'horizontal':  '横向海湾（东西向海峡）',
    'archipelago': '近海群岛（破碎海岸 + 岛屿 + 近岸水道）',
    'none':        '无海洋（全陆地）',
}


def make_ocean(n, rng, cfg):
    """海陆几何模板（coast 因子）。返回 (ocean, meta)；ocean 为 None 表示全陆地。

    - vertical / horizontal：正弦叠加的蜿蜒海峡（vertical 与原实现逐像元一致）。
    - archipelago：带限随机场阈值化得到破碎海岸，再叠一条近岸水道（固定模板）。
    - none：无海洋。
    """
    coast = cfg.get('coast', 'vertical')
    if coast not in COASTS:
        raise ValueError(f"Unknown coast {coast!r}; expected one of {sorted(COASTS)}")
    if coast == 'none':
        return None, dict(coast=coast, ocean_frac=0.0)
    yy, xx = np.mgrid[0:n, 0:n]
    jitter = bool(cfg.get('coast_jitter', False))
    if coast in ('vertical', 'horizontal'):
        amp = cfg.get('ocean_amp', 0.10) + (rng.uniform(-0.02, 0.02) if jitter else 0.0)
        amp2 = cfg.get('ocean_amp2', 0.05) + (rng.uniform(-0.01, 0.01) if jitter else 0.0)
        wid = cfg.get('ocean_width', 0.05) + (rng.uniform(-0.01, 0.01) if jitter else 0.0)
        phase = cfg.get('ocean_phase', 1.3) + (rng.uniform(0, 2*np.pi) if jitter else 0.0)
        along, across = (yy, xx) if coast == 'vertical' else (xx, yy)
        center = (n / 2.0
                  + amp * n * np.sin(2 * np.pi * along / (n * 0.6))
                  + amp2 * n * np.sin(2 * np.pi * along / (n * 0.23) + phase))
        ocean = np.abs(across - center) < n * wid
    else:  # archipelago
        rif = np.random.default_rng(int(cfg.get('arch_seed', 20260912)))
        f = matern_field(n, rif, cfg.get('arch_len_scale', 7.0))
        ocean = f > np.quantile(f, 1.0 - float(cfg.get('arch_ocean_frac', 0.25)))
        chan = float(cfg.get('arch_channel', 0.18))
        ocean |= np.abs(xx - (chan * n + 0.06 * n * np.sin(2 * np.pi * yy / (n * 0.5)))) \
            < n * float(cfg.get('arch_channel_w', 0.05))
    return ocean, dict(coast=coast, ocean_frac=float(ocean.mean()))
...
def gen_landscape(n, rng, cfg=None):
    ...
    if cfg.get('nonstationary', False):
        X1 = nonstationary_matern_field(n, rng, short_scale=cfg.get('short_scale', 5.0),
                                        long_scale=cfg.get('long_scale', 25.0))
        X2 = nonstationary_matern_field(n, rng, short_scale=cfg.get('short_scale', 6.0),
                                        long_scale=cfg.get('long_scale', 20.0))
        X3 = nonstationary_matern_field(n, rng, short_scale=cfg.get('short_scale', 7.0),
                                        long_scale=cfg.get('long_scale', 18.0))
    else:
    ...
    ocean = None
    if cfg.get('ocean'):
        ocean, _coast_meta = make_ocean(n, rng, cfg)   # 海岸几何由 coast 因子决定
...
def gen_labels(p_true, labels, rng, cfg):
    ...
    if cfg.get('observation_model') == 'detection':
        # Spatially varying reporting probability (surveillance capacity proxy).
        n = p_true.shape[0]
        yy, xx = np.mgrid[0:n, 0:n]
        q = cfg.get('detect_min', 0.35) + (cfg.get('detect_max', 0.95)-cfg.get('detect_min',0.35)) * (0.5+0.5*np.sin(xx/n*3*np.pi)*np.cos(yy/n*2*np.pi))
        p_eff = np.clip(p_true.ravel() * q.ravel(), 0.001, 0.999)
...
GENERATIONS = {
    ...
    'g_nonstationary': dict(name='非平稳空间相关长度', mode='bernoulli',
                      noise_base=0.12, noise_diff=0.18, ridge=3.0, len_scale=18.0,
                      nonstationary=True, short_scale=5.0, long_scale=28.0,
                      frag=1.0, ocean=True),
    'g_detection': dict(name='空间检测能力异质性', mode='bernoulli',
                      observation_model='detection', ridge=3.0, len_scale=18.0,
                      frag=0.0, ocean=True),
}
PARTITIONS = {
