...
LogLoss      0.5991±0.0686      0.5962±0.0688      0.5950±0.0682
...
LogLoss      0.6101±0.0629      0.6062±0.0632      0.6048±0.0624
------------------------------------------------------------------
配对显著性（每模拟 Δ = ASR − CE；配对 t + Wilcoxon，n=90）:
  ...
  空间CV LogLoss  Δ-0.0053±0.0027  t=-18.26  p<0.0001  Wilcoxon p<0.0001
  测试集 Iso ratio Δ-0.1085±0.0594  t=-17.25  p<0.0001  Wilcoxon p<0.0001
------------------------------------------------------------------
按 生成 分档（空间CV AUC 增益 / λ* 均值）:
  g_clean (简单)  ΔAUC +0.0038   λ*均值 1.23
  g_mid   (中等)  ΔAUC +0.0054   λ*均值 1.60
  g_hard  (困难)  ΔAUC +0.0081   λ*均值 2.17
------------------------------------------------------------------
要点：全部模型按 7:2:1 划分 + 验证集早停（上限 1000 轮，与论文配置一致）。
三档难度下 ASR 相对 CE 的增益随难度单调递增（空间CV ΔAUC +0.0038 / +0.0054 /
+0.0081，配对检验全显著），判别精度从不退化；空间指标（Moran's I / ContED /
Iso ratio，与论文口径一致）全面改善（困难档孤立像元比例降 17.4 个百分点）；
门控带 CE∈[0.4,0.6] 增益约为对应档全局的 2~8 倍（验证门控设计）；λ* 随数据
难度自适应（均值 1.23 / 1.60 / 2.17）。
