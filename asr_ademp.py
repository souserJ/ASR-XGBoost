#!/usr/bin/env python3
"""ADEMP simulation runner. Legacy asr_demo_modified.py remains a reference.
...
import asr_demo_modified as legacy
...
def generate_data(args, scenario, rep):
    seeds = {key: seed_for(args.seed, scenario, rep, key)
             for key in ("landscape", "dgp_regions", "observation", "sample")}
    rng = lambda key: np.random.default_rng(seeds[key])
    cfg = legacy.GENERATIONS[scenario]
...
def model_regions(data, args, scenario, rep, partition):
    rng = np.random.default_rng(seed_for(args.seed, scenario, rep, "model_regions", partition))
    if partition == "p_grid":
...
def summarize(rows):
    ...
    summary, paired = [], []
    conditions = sorted({(r["scenario"], r["partition"], r["validation"]) for r in rows})
    for condition in conditions:
        subset = [r for r in rows if (r["scenario"], r["partition"], r["validation"]) == condition]
        base = dict(zip(("scenario", "partition", "validation"), condition))
...
def parse_args(argv=None):
    ...
    p.add_argument("--out", type=Path, default=Path(__file__).parent/"results"/time.strftime("ademp_%Y%m%d_%H%M%S"))
    a = p.parse_args(argv)
    a.gens, a.parts, a.validation = ([x.strip() for x in s.split(",")] for s in (a.gens, a.parts, a.validation))
    a.lam_grid = sorted(set(float(v) for v in a.lam_grid.split(",")))
    for value, valid in ((a.gens, legacy.GENERATIONS), (a.parts, legacy.PARTITIONS), (a.validation, ("random", "spatial"))):
...
def main(argv=None):
    ...
    for scenario in args.gens:
        for rep in range(args.reps):
            try:
                data = generate_data(args, scenario, rep)
            except Exception as exc:
                failures.append(dict(scenario=scenario, rep=rep, partition="all", validation="all", error=repr(exc)))
                write_csv(args.out/"failures.csv", failures)
                print(f"FAIL {scenario} rep={rep}: {exc}", flush=True)
                continue
            seeds.append(dict(scenario=scenario, rep=rep, **data["seeds"]))
            np.savez_compressed(args.out/f"truth_{scenario}_{rep}.npz", p_true=data["p_true"], p_obs=data["p_obs"],
            ...
            for partition in args.parts:
                for protocol in args.validation:
                    context = dict(scenario=scenario, partition=partition, validation=protocol, rep=rep)
                    try:
                        labels = model_regions(data, args, scenario, rep, partition)
                        ...
                        outer_seed = seed_for(args.seed, scenario, rep, protocol, "outer")
                        ...
                        np.savez_compressed(args.out/f"pred_{scenario}_{rep}_{partition}_{protocol}.npz",
                                            sample=data["sample"], fold=fold_map, model_regions=labels, **preds)
                        print(f"OK {scenario} rep={rep} {partition} {protocol} ({time.monotonic()-start:.1f}s)", flush=True)
                    except Exception as exc:
                        failures.append(dict(scenario=scenario, rep=rep, partition=partition, validation=protocol, error=repr(exc)))
